...
#SBATCH -o /bigdata/cutlerlab/jjaco081/PYR1_pocket_expansion/logs/posechk_%a.log
#
...
[[ -s $PC/$T/system.prmtop ]] || { echo "no system for $T"; exit 1; }
mkdir -p $PC/$T/$REP
RUNS=( "$PC/$T|$PC/$T/$REP" )
echo "=== pose-check $T $REP  ${PROD_NS} ns  $(date -Is)"
...
for entry in "${RUNS[@]}"; do
    ...
    if [[ "$REMAIN_PS" -le 0 ]]; then
        ...
        echo "PC_RUN_DONE dir=$D rc=0 time=$NOW/$TARGET_PS"
        continue
    fi
    ...
    echo "PC_RUN_DONE dir=$D rc=$rc time=${FIN:-unknown}/${TARGET_PS} ps"
    [[ $rc -ne 0 ]] && overall=1
done
