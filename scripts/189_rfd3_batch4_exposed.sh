...
#SBATCH -t 1:58:00
#SBATCH -J rfd3b4
#SBATCH -o /bigdata/cutlerlab/jjaco081/PYR1_pocket_expansion/logs/rfd3b4_%j.log
#
...
#    PYR1 with new loops.
#
# ⚠ BATCH 4: THE CONDITIONING FLIPPED. If a pocket-lining residue is genuinely
# water-accessible -- which it is, that is what a pocket means -- then the
# correct request is select_EXPOSED (rasa_bin=2), not buried. Batch 2 asked for
# the opposite of what a pocket is. Same 12 residues, opposite bin.
#
# (batch 3 rationale, kept) NO BURIAL CONDITIONING. Jannis's concern, and the code confirms the
...
echo "contig: $CONTIG"
echo "exposed: $BURIED   (the SAME residues, conditioned EXPOSED not buried)"
...
rfd3 \
  ...
  +specification.select_exposed=\'$BURIED\' \
