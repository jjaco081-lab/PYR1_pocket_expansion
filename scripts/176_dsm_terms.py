#!/usr/bin/env python
r"""
176_dsm_terms.py -- the WIN DSM library rescored PER TERM.

INDEPENDENT TEST of README 100b. fa_sol reached AUC 0.682 on fludioxonil, a
ligand that already fits, where the total ddG is at chance (0.550) and a plain
side-chain VOLUME lookup gives 0.549. That is one compound, one lab, one library.

WIN 55,212-2 is the right transfer case: a different ligand class (cannabinoid),
a different lab (Beltran), a different library design (DSM doubles), a crystal
pose (7MWN) rather than a predicted one -- and, per README 94c, it is a FITTING
ligand, straining wild-type PYL2 by only +15.7 REU. So the total ddG should fail
here as it did on fludioxonil (89a: median rank 6632 of 35,863, p = 0.047), and
the question is whether fa_sol does better.

PRE-REGISTERED: fa_sol must beat both the total ddG AND the volume baseline on
the five round-1 hits, or the fludioxonil result does not transfer.
...
OUT = os.path.join(ROOT, "results", "dsm_terms")
...
def main():
    ...
    TERMS = ["fa_rep", "fa_atr", "fa_sol", "lk_ball_wtd", "fa_elec", "hbond_sc"]
    st = {t: getattr(ScoreType, t) for t in TERMS if hasattr(ScoreType, t)}
    lp = pyrosetta.pose_from_pdb(LIGPDB); sfxn(lp)
    e_lig_t = {t: lp.energies().total_energies()[x] for t, x in st.items()}

    def terms_of(p):
        sfxn(p)
        E = {t: p.energies().total_energies()[x] for t, x in st.items()}
        q = p.clone(); q.delete_residue_slow(q.total_residue()); sfxn(q)
        return {t: E[t] - q.energies().total_energies()[st[t]] - e_lig_t[t]
                for t in st}
    ...
    def repack(p0, allowed, nrep):
        ...
        for _ in range(nrep):
            ...
            v.append(dg(p))
            T = terms_of(p)
            if best is None or T["fa_rep"] < best["fa_rep"]:
                best = T
        return min(v), float(max(v) - min(v)), best
