...
#SBATCH -o /bigdata/cutlerlab/jjaco081/PYR1_pocket_expansion/logs/md_pre_%a.log
#
# 42_md_run_preempt.sh -- S4_ternary only, on preempt_gpu.
#
...
#
# COORDINATION WITH JOB 27333712
#   Array tasks 9/10/11 of that job are the same three S4 replicates and write
#   the same directories. They are held (scontrol hold), NOT cancelled, so they
#   keep their queue position. If this works, cancel them; if it does not,
#   release them. Never let both run at once -- two pmemd processes writing one
#   prod.rst7 will corrupt it.
set -uo pipefail
