...
#SBATCH -t 1:58:00
#SBATCH -J rfd0b
#SBATCH -o /bigdata/cutlerlab/jjaco081/PYR1_pocket_expansion/logs/rfd0b_%j.log
#
...
#    PYR1 with new loops.
#
# ⚠ BATCH 3: NO BURIAL CONDITIONING. Jannis's concern, and the code confirms the
# mechanism. rfd3/transforms/rasa.py:44 bins RASA with a 1.4 A (water) probe at
# edges [0.0, 0.1, 0.2]: bin 0 = buried, 1 = partially buried, 2 = exposed. A
# residue lining a LARGE pocket is water-accessible, so its true RASA is HIGH and
# it belongs in bin 2. Asking select_buried (rasa_bin=0) demands RASA < 0.1 --
# essentially no water access -- which the model can only satisfy by packing
# protein around those residues and ELIMINATING the pocket. That is consistent
# with batch 2's 9-of-10 zero-cavity outcome.
# Everything else is identical to batch 2, so this isolates the variable.
#
# (superseded) 3. BURIAL CONDITIONING instead of secondary-structure bias. RFd3 dialect 2
...
# STAGE 0b -- 0a PLUS select_partially_buried. RASA bin 1 is 0.1-0.2, the
# intermediate case; bin 0 (buried) collapsed the pocket to 1/10 and bin 2
# (surface-exposed) gave 0/10. This is the correctly-binned arm.
# STAGE 0a -- anchor the N-terminal beta strand and shorten the leading range.
# Batch 3 left a 50-70 residue UNANCHORED leading segment; 46 of the first 60
# design residues ended up within 4.5 A of HAB1 (the tail Jannis saw wrapping it)
# and it hosted the recurring pocket-spanning lysines (K57 in 4 designs, K63 in 3).
# PYR1 34-40 is 100% beta strand by P-SEA, so anchoring it gives the sheet a
# nucleus instead of a free tail. 61 fixed residues, all 19 HAB1-interface
# residues still covered (asserted before submission).
CONTIG="10-25,A34-40,10-20,A58-65,15-30,A81-92,20-35,A111-121,25-40,A146-168,5-15,/0,B185-221,/0,B232-270,/0,B283-461,/0,B466-505"
...
echo "contig: $CONTIG"
echo "partially_buried: $BURIED   (RASA bin 1 -- the INTERMEDIATE case a pocket wall occupies)"
...
rfd3 \
  ...
  +specification.select_partially_buried=\\'$BURIED\\' \\
