...
COLS = ["Method", "Workstream", "What it computes (how)", "How we used it",
        "Ligands / structures", "Dates (2026)", "Key result",
        "Shortcomings & failures", "What we learned", "Status",
...
["Cavity characterisation (lib_cavity)", "Pocket - structure",
 ...
 "F108 is the lobe gatekeeper. No inter-lobe bottleneck exists - the limit is ligand LENGTH. Satellite lobe sealed in 7/7 chains.",
...
["Foldseek + CATH50 superfamily scan", "Pocket - structure",
 ...
 "OPTION", "CPU cutlerlab", "03, 05, 08, 10"],
...
["Rosetta open-vs-closed ddG (Arm 2 / 2b)", "Pocket - energetics",
 ...
 "OPTION", "CPU cutlerlab", "22, 26, 32, 33; jobs 27275686, 27286483/4"],

["Corrected discrimination statistic", "Pocket - statistics",
 "F = s2_between / (s2_within / n) - replicate-mean variance, not raw replicate sd.",
 "Re-derived every arm's discrimination after finding the original statistic wrong.",
 "All Rosetta arms", "Aug 10",
 "Flipped Arm 2b from 'blind' to F = 9.0, correcting a central project claim.",
 "The original denominator ignored sqrt(n); the error had already propagated into several conclusions.",
...
["LigandMPNN", "Pocket - design",
 ...
 "Mandipropamid (3UZ, 4WVO) vs ABA (A8S, 3QN1) as ligand-swap null", "Aug 12",
...
["Rosetta FastDesign", "Pocket - design",
 ...
 "OPTION", "CPU cutlerlab", "50, 51; job 27421344"],
...
["Coupled moves (Kortemme)", "Pocket - design",
 ...
 "OPTION", "CPU cutlerlab, 100 tasks", "74, 74b; job 27560390"],
...
["Per-position steric admissibility", "Pocket - geometry",
 ...
 "Every miss was a GROW mutation. Real sensors are compensating shrink/grow PAIRS (F159A+A160I; Y120G+A160G in 27 and 23 of Beltran's 45 sensors), which a per-position filter structurally cannot see.",
...
["Electrostatic / H-bond complementarity", "Pocket - geometry",
 ...
 "OPTION", "CPU local", "73_electrostatic.py"],
...
["Coumarin focused-library benchmark", "Pocket - benchmark",
 ...
 "Tian sd04: Coumarin, PFAS, TNTv1/v2 libraries; 692 characterised clones", "Aug 14",
...
["Beltran cannabinoid sensor table", "Pocket - benchmark",
 ...
 "WIN 55,212, JWH series, D9-THC, CBDA, CP 47,497", "Aug 18-19",
...
["Literature protocol review", "Pocket - benchmark",
 ...
 "Leonard 2026, An 2024, Tian 2025, Park 2023, ACS Chem Biol 2024, Nat Biotech 2026", "Aug 19",
...
["Conformation x occupancy factorial", "MD",
 ...
 "ACTIVE (running)", "GPU", "69, 69b, 70; job 27547457"],
...
["BindCraft", "Binder design",
 "Hallucination-based binder design against a target surface.",
 "Generated candidate replacements for HAB1 as the PYR1 readout partner.",
 "PYR1 closed-state surface; 2,950 structures", "pre-Aug",
 "Largest and most reliable design set; ~80% alpha-helical. Dock rate 91%.",
 "Strong fold bias toward helical bundles; low structural diversity.",
 "Design-set composition matters: helical bias limits how much interface space is explored.",
 "ACTIVE", "GPU", "-"],

["RFdiffusion", "Binder design",
 "Diffusion model generating backbones conditioned on a target.",
 "Second, more structurally diverse design set.",
 "PYR1; 193 structures", "pre-Aug; re-tested Jul 23",
 "~64% 'Other'/diverse folds - much broader than BindCraft.",
 "Boltz screen results at n=193 went stale and needed re-testing. Dock rate only 9% vs BindCraft's 91%.",
 "Structural diversity and dock reliability trade off against each other.",
 "OPTION", "GPU", "-"],

["BoltzProt-1", "Binder design",
 "Generative protein design against a target.",
 "Third design set.",
 "PYR1; 100 designs", "pre-Aug",
 "Highest dock rate of the three sets (95%).",
 "Worst Rosetta ddG of the three (median +10.6 vs BindCraft +7.2, RFdiffusion +3.9).",
 "Dock rate and interface energy are not the same axis and can disagree sharply.",
 "OPTION", "GPU", "-"],

["ESMFold2 re-prediction", "Binder design - validation",
 "Structure prediction with a ligand input; iPTM as an interface-confidence score.",
 "Re-scored the design sets and the sequenced clone across seeds.",
 "PYR1 + ABA (A8S) + binder; 193 designs x 3 seeds", "pre-Aug",
 "iPTM is reproducible at the SET level: ICC(1) = 0.735.",
 "THE POSE IS NOT reproducible - 13-18 A across seeds. 20/193 designs unstable; 804_1 is the most unstable, strictly bimodal (0.455 vs 0.889). seed=0 does not repeat run-to-run.",
 "A stable iPTM of ~0.50 means 'reliably no dock'. Report a DOCK RATE, never an average of a bimodal distribution. Every single-structure Rosetta metric downstream is a draw from that distribution.",
 "ACTIVE", "GPU", "-"],

["Rosetta ddG on designs (OvC)", "Binder design - scoring",
 "FastRelax + interface ddG on the top-iPTM structure per design.",
 "Ranked 872 designs, anchored against real HAB1 and the PYR1 dimer.",
 "PYR1 + ABA + binder; 872 designs", "pre-Aug",
 "Complete (872/872). Anchors behaved: HAB1 +1.8 to +3.6, PYR1 dimer -9 to -36.",
 "Scored on a single structure drawn from an irreproducible pose distribution. Two Rosetta failures may indicate extreme closed-PYR1 selectivity rather than error.",
 "Anchor every score against known positives and negatives before reading the ranking.",
 "ACTIVE", "CPU cutlerlab", "-"],

["Boltz-2 screen", "Binder design - scoring",
 "Co-folding plus interface confidence screen.",
 "Screened design sets against PYR1.",
 "PYR1 + designs", "Jul",
 "BindCraft results looked solid.",
 "WT HAB1 CALIBRATION FAILS: Boltz BC/minPAE badly under-score the real, known-good HAB1 interaction, so the screen cannot be trusted as an absolute filter.",
 "Always calibrate a screen against the known true positive. A screen that cannot recognise HAB1 cannot rank HAB1 replacements. ~$13 per screen; not re-run.",
 "OPTION", "cloud (paid)", "boltz skills"],

["Y2H experimental validation", "Binder design - experiment",
 "Yeast two-hybrid growth and reporter assay for ligand-dependent interaction.",
 "Tested the designed binder 804_1 against PYR1.",
 "PYR1 + ABA + 804_1; Sanger-verified clones", "Aug 13",
 "804_1 BINDS - the design works. Roughly 100x weaker than HAB1.",
 "100x = 2.8 kcal/mol. The MRSL scar does not hurt; the GAL4AD FUSION is what breaks docking.",
 "2.8 kcal/mol is 3-5 mutations - routine affinity maturation. SC 0.56 is the headroom; hydrophobicity 0.41 is NOT unusual (56th percentile).",
 "ACTIVE", "wet lab", "sequences/"],
]
...
def write_xlsx(path):
    ...
    for r in ROWS:
        ws.append(r)
    widths = [30, 20, 62, 46, 40, 16, 62, 62, 62, 16, 22, 34]
    ...
    for st in order:
        ...
        for r in rows:
            ...
            out += [f"### {d['Method']}", "",
                    ...
                    f"- **Ligands / structures:** {d['Ligands / structures']}",
