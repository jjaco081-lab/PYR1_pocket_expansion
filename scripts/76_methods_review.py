...
ROWS = [
["RFdiffusion3 motif scaffolding", "Scaffold - design",
 "Diffusion scaffolding around the four HAB1-interface segments held as a fixed motif, with HAB1 present as a steric constraint and sampled scaffold segment lengths.",
 "The only untested route to a genuinely larger BACKBONE, after 106 showed no natural donor offers one.",
 "PYR1 chain A + HAB1 chain B from 3QN1, auth-numbered", "Sep 1-14",
 "The motif scaffolds easily -- 0.1 A CA deviation over the fixed positions, all four segments present in 10/10. Batch 3 (no RASA conditioning) gives 7/10 designs with an enclosed cavity and ALL SEVEN exceed PYR1's 1184 A^3 backbone envelope, best 1353 (+14%).",
 "RASA conditioning SUPPRESSES the pocket, which Jannis diagnosed from the design and the code confirms: rasa.py bins with a 1.4 A water probe, so a pocket-lining residue is genuinely high-RASA and select_buried (bin 0) is satisfied by packing protein around it. buried 1/10, none 7/10, exposed 0/10. Designs are still loose (Rg 22-27 A vs PYR1's 15.0) and the sequence is a low-complexity placeholder (71% A/S/T/G/P/V), so every volume is a polyGly number.",
 "Four invocation traps, all recorded in the script header: comma-separated contigs, /0 chain breaks, top-level specification fields, and quoted comma values. Feed an auth-numbered PDB -- RFd3's CIF parser reads label numbering and 3QN1 chain A carries a -2 offset.",
 "ACTIVE (running)", "-", "GPU short_gpu", "183, 187, 188, 189, 193; jobs 28380382/3"],

["Structural filter cascade with a PYR1 gate", "Design - filtering",
 "Seven filters on scaffold backbones -- enclosed cavity, largest-CONNECTED polyGly envelope, motif integrity, HAB1-wrapping, pocket-spanning residue, compactness, depth/width -- with wild-type PYR1 required to pass first.",
 "Jannis's condition: a filter that rejects wild-type PYR1 is miscalibrated, not strict.",
 "PYR1 3QN1 frame; RFd3 batches 1-3", "Sep 14",
 "The gate works. On first use F5 (no pocket-spanning residue, defined as a CB within 4 A of the cavity axis) REJECTED wild-type PYR1, flagging five ordinary wall residues, because PYR1's chamber is 11.6 A long so every lining residue qualifies. F5 is now a merge test -- a residue splits the cavity if truncating it merges components -- and PYR1 passes all five.",
 "Fifth metric of mine to be wrong in this project, and the first caught BEFORE being applied to anything.",
...
["Ligand-conditioned priors (four attempts)", "Design - library",
 ...
 "194 sd03 ligands; Beltran-45 and sd07 as test sets", "Aug 24 - Sep 14",
 "All four FAIL out of sample. The ligand-blind frequency prior remains unbeaten (recall@20 0.429, @40 0.660 on Beltran-45).",
 "The size arm looked like a win -- +0.142 recall@20, sign-flip p 0.0017, 10 wins 0 losses -- and was RETRACTED by two controls: the small arm loses 0/7 on sd07, and 400 random subsets of equal size reproduce the large-arm gain (P = 0.21). The paired test was correct but measured SUBSETTING, and a one-arm test cannot see a routing effect.",
 "Control against a RANDOM subset of the same size before crediting any conditioning. And no clean small-ligand test exists: every Tian downstream library was drawn from the sd03 vocabulary (sd07 100%, sd08 96%, sd09 67%), so check the LIBRARY's provenance, not the compounds'.",
 "RETIRED", "-", "CPU local", "177, 178, 185, 194"],

["Ligand size ceiling", "Design - benchmark",
 "Hit rate vs heavy-atom count over sd01's 194 hits and 3,172 documented failures, with cLogP, TPSA and rotatable bonds fitted jointly.",
 "The premise the whole pocket-expansion project rests on, never previously checked against the screening record.",
 "sd01, 3,366 parsed ligands", "Sep 14",
 "The ceiling is real and sharp. The largest ligand that ever produced a sensor is 36 heavy atoms; of 222 tested at >= 40, NONE are hits (p = 1.9e-6 at the overall 5.8% rate). Hit rate falls 6-fold from the 15-25 atom plateau to the 30-40 band. Heavy atoms stays z = -5.22 with the confounds in the model.",
 "A ceiling is consistent with pocket-limitation AND with large compounds not reaching the yeast nucleus; this cannot separate them.",
 "945 documented failures sit in the 28-50 atom band -- targets where wild-type PYR1 demonstrably fails, so each carries its own negative control.",
 "ACTIVE", "-", "CPU local", "186"],

["Ligand-conditioned substitution priors", "Design - library",
 "Similarity-weighted priors over substitutions: whole-molecule ECFP4/descriptor aggregation, and a per-pocket-position pharmacophore version.",
 "Jannis's MSA analogy -- aggregate everything known about every screened ligand to BIAS the choice among sequences other filters allow, rather than to predict.",
 "194 sd03 ligands with clones; 181 with co-folded poses", "Sep 1",
 "The whole-molecule prior beats the ligand-blind bar by +0.05 recall@20 (0.398 vs 0.345), five times 52's +0.010, and is STABLE across the weighting exponent rather than sitting on a knife edge. Paired: +0.053, t = 17.4, 49 of 50 splits positive.",
 "The control kills the MSA analogy: the entire gain comes from ligands WITH a close analog (+0.191 at Tanimoto >= 0.5) and it is NEGATIVE where none exists (-0.036 at < 0.2) -- the case a novel target is in. The per-position version fails at every shell radius and monotonically worse the more local it gets, because the pocket is 11.6 A long so a 7 A shell is the whole molecule, and a 3.5 A shell leaves too few ligands per position.",
 "Gate it on max Tanimoto >= 0.25: +0.056 across 69 % of ligands, switched off for novel chemotypes. A data-density limit, not a flaw in the idea -- 18 position-specific priors cannot be estimated from 182 ligands.",
 "ACTIVE", "-", "CPU local", "177, 178"],

["Backbone envelope vs cavity volume", "Pocket - structure",
 "polyGly free volume inside a structurally-aligned wall hull, plus depth and depth/width of that envelope. Never cavity-derived.",
 "Jannis: cavity volume conflates backbone with side chains, and if a donor's larger cavity is just smaller side chains that is cheaper to get by mutating PYR1.",
 "PYR1 + 6 SRPBCC donors + 3 START lipid-transfer domains", "Sep 1",
 "It overturned the donor ranking. 2PCS -- preferred throughout this project -- has a polyGly volume of 1173 against PYR1's 1184, DEAD EVEN: its 570 A^3 cavity against PYR1's 164 is entirely side chains. Same for 2NS9, 6AWV, 3OQU. The real difference is SHAPE: PYR1's envelope is nearly isotropic (d/w 1.25) while CERT/2E3M is 36.8 A deep at d/w 2.36.",
 "This metric was wrong twice before (143's circular envelope, 143's poly-Gly column reporting a vanished component as 0-3 A^3). 145 fixed both and this uses that version.",
 "Since 8's limit on PYR1 is LENGTH not volume, the importable difference is a longer pocket, not a bigger one -- and no donor offers both. Design targets should be depth and d/w.",
 "ACTIVE", "-", "CPU local", "181, 182"],

["Pose-check MD", "Structure - validation",
 "6 ligand/receptor systems x 3 replicates x 20 ns; ligand heavy-atom RMSD to the starting pose with the protein core superposed.",
 "Asks whether MD retains a predicted pose, since most poses this project trains on were predicted once and 93a showed co-folding places a ligand in the pocket regardless of whether it belongs.",
 "ABA, mandipropamid (crystal AND predicted), WIN, fludioxonil, benoxacor", "Sep 1",
 "IN FLIGHT, 8 of 18 complete. All six systems built at 39,507-43,893 atoms with no parmchk2 ATTN flags -- no guessed parameters, which matters because a guessed torsion is indistinguishable from a bad pose in an RMSD trace.",
 "Six build attempts were needed, every failure a silent format mismatch (explicit hydrogens, sanitize=False, MDL aromatic bond type 4). Recorded in the script so they are not rediscovered.",
 "Two paired comparisons carry it: mandipropamid crystal vs predicted, and fludioxonil (0.71 A seed spread) vs benoxacor (3.27 A).",
 "ACTIVE (running)", "-", "GPU short_gpu", "179, 180; job 27992285"],

["RFdiffusion3 motif scaffolding", "Scaffold - design",
 "Diffusion scaffolding around the four HAB1-interface segments held as a fixed motif, with wide linker ranges so scaffold length and therefore envelope can vary.",
 "Revisits 7's rejection. That objection was about the two-state switch, NOT about cavity generation, and 85 removed its discriminating power -- no design of any kind can now be shown to cycle.",
 "PYR1 chain A, residues 1-181; motif A60-63/A84-89/A115-117/A148-166", "Sep 1",
 "QUEUED behind the pose-check MD. Residue numbering asserted before submission: all 19 interface residues present with expected identities, gaps only at 69-70 which fall in a rebuilt linker.",
 "Fixing the C-lobe span freezes F159/V163/V164, which are pocket wall, so depth gain is one-sided. Inherent to preserving the interface. A prior RF3 attempt in the sibling project failed on a config API mismatch.",
 "Scored on ENVELOPE not cavity volume (106). Batch 2 is specified with Jannis's minimal loop anchors: 54 residues covering all 19 interface positions, against 139 for the literal extend-to-structure reading.",
 "ACTIVE (running)", "-", "GPU short_gpu", "183; job 27994380"],
