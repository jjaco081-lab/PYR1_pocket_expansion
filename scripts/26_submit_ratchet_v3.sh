...
#SBATCH --mem=12G
#SBATCH -t 7-00:00:00
#SBATCH -J pe_ratchet3
#SBATCH -a 0-5
#SBATCH -o /bigdata/cutlerlab/jjaco081/PYR1_pocket_expansion/logs/ratchet3_%a.log
#
# 22_submit_ratchet_v3.sh -- Arm 2 re-run at nstruct=20 with the gate and latch
# released from coordinate constraints (see 25_rosetta_ratchet_v3.py).
#
# SHAPE OF THE JOB
...
mkdir -p "$CLAIMS" "$OUT/pdb" "$P/logs/units_v3"
...
run_unit() {
    ...
    if $PY "$S/25_rosetta_ratchet_v3.py" \
           --index "$v" --nstruct 1 --rep-offset "$r" --chunk "$r" \
           >"$P/logs/units_v3/unit_v${v}_r${r}.log" 2>&1; then
...
echo "RATCHET3_TASK_DONE id=$i rc=$?"
date
