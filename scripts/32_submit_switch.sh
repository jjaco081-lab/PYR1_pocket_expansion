...
#SBATCH -J pe_switch
#SBATCH -a 0-5
#SBATCH -o /bigdata/cutlerlab/jjaco081/PYR1_pocket_expansion/logs/switch_%a.log
#
# 22_submit_switch.sh -- Arm 2 re-run at nstruct=20 with the gate and latch
# released from coordinate constraints (see 21_rosetta_switch.py).
#
# SHAPE OF THE JOB
...
S=$P/scripts
OUT=$P/results/switch
CLAIMS=$OUT/claims

NVAR=41          # panel size, must match variants.py
NREP=12          # replicates per variant (2 relaxes each: closed + open)
WORKERS=8        # parallel relax processes per array task (= #SBATCH -c)

mkdir -p "$CLAIMS" "$OUT/pdb" "$P/logs/units_switch"
...
run_unit() {
    ...
    local name=${names[$v]}
    local json="$OUT/${name}__r${r}.json"

    [[ -s "$json" ]] && return 0                       # already done
    mkdir "$CLAIMS/v${v}_r${r}" 2>/dev/null || return 0 # claimed by someone else

    if $PY "$S/31_rosetta_switch.py" \
           --index "$v" --rep "$r" --nstruct 1 \
           >"$P/logs/units_switch/unit_v${v}_r${r}.log" 2>&1; then
...
TOTAL=$((NVAR * NREP))
# REP-MAJOR ordering: every variant at rep 0, then every variant at rep 1, ...
# If the array is starved of cores, the partial result is then a COMPLETE panel
# at low replication rather than a few variants sampled deeply.
for ((k = 0; k < TOTAL; k++)); do
    j=$(((k + i * TOTAL / 6) % TOTAL))
    echo "$((j % NVAR)) $((j / NVAR))"
done | xargs -P "$WORKERS" -n 2 bash -c 'run_unit "$0" "$1"'

echo "SWITCH_TASK_DONE id=$i rc=$?"
date
