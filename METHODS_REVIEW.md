...
| **Aug 31** | External audit finds the factorial extraction bug; 62a is recomputed on the restored 1.6 us and RETRACTED -- the dF sign flips across replicate pairings. |
| **Aug 31** | Park's Figure 5 decoded: 7,045 true negatives, the first in the project. AUC 0.868 on mandipropamid. |
| **Aug 31** | Clash relief is not specific: azoxystrobin and lufenuron clash as hard as mandipropamid and have zero responders of 475. |
| **Sep 1** | The volume baseline, applied retroactively: side-chain volume alone gives 0.781 on mandipropamid, so the whole structural pipeline buys ~0.09. |
| **Sep 1** | Desolvation reaches fludioxonil (0.682 where the total is 0.550) and then FAILS to transfer to the WIN library. |
...
| **Sep 1** | Backbone envelope overturns the donor ranking -- 2PCS's polyGly volume equals PYR1's, so its cavity is side chains. |
...
| `3KDI / 3KDJ` | Further deposited PYR1/PYL chains. | Additional apo/holo chains in the satellite-lobe survey. |
| `3NS2` | PYL/PYR-family receptor. | Superfamily comparison in the helix-grip survey. |
| `7ZTQ / 7ZVR` | PYR1-family complexes. | Cross-checks on pocket geometry outside the 3QN1/3K3K pair. |
| `2PCS` | CoxG, a bacterial SRPBCC protein (CATH domain 2pcsA00). | THE ONLY homolog with a genuinely large SINGLE chamber: 570 A^3 vs PYR1's 119, no satellites, bottleneck r_max 3.80 A. It lines that volume with 27 side chains vs PYR1's 19-20, and inserts only 2 residues - so the volume is side-chain packing, NOT new backbone. |
| `8UDS` | CoxG, second structure. | Explains why the best donor is unusable: CoxG EXTRACTS MENAQUINONE OUT OF THE LIPID MEMBRANE and returns it, so loading is mechanistic rather than diffusive and the pocket is OPEN - incompatible with a closed-state readout. |
| `2NS9` | Helix-grip homolog, 455-462 A^3 total. | Looks large but is FRAGMENTED - 157 + 114 + 66 + 127 A^3, main chamber only 157. APO structure. |
| `2BK0` | Helix-grip homolog, 344-346 A^3 total. | Also fragmented (172 main + 60/103/12). APO structure. |
| `6AWV` | Helix-grip homolog, 305-319 A^3 total. | Main chamber only 126 A^3, and its satellites sit behind a 1.43 A bottleneck - narrower than a water probe. |
| `3OQU` | PYR1-family relative proposed as an enlargement candidate. | Has FEWER real ABA contacts than PYR1 (16/32 vs 19/24); its 204 A^3 total is 146 main plus a 58 A^3 satellite behind r = 2.23. |
| `4DSB` | Helix-grip homolog, 173 A^3 single chamber. | Proposed as a donor on chamber size and REJECTED by Jannis, correctly: on ligand-free measures it has LESS room than PYR1 (envelope 1804 vs 1873, polyGly ceiling 1079 vs 1184) and its variable positions sit where PYR1's already are. |
...
### Structural filter cascade with a PYR1 gate

**Design - filtering** · Sep 14 · CPU local · `190, 192`

- **How it computes:** Seven filters on scaffold backbones -- enclosed cavity, largest-CONNECTED polyGly envelope, motif integrity, HAB1-wrapping, pocket-spanning residue, compactness, depth/width -- with wild-type PYR1 required to pass first.
- **How we used it:** Jannis's condition: a filter that rejects wild-type PYR1 is miscalibrated, not strict.
- **Inputs:** PYR1 3QN1 frame; RFd3 batches 1-3
- **Key result:** The gate works. On first use F5 (no pocket-spanning residue, defined as a CB within 4 A of the cavity axis) REJECTED wild-type PYR1, flagging five ordinary wall residues, because PYR1's chamber is 11.6 A long so every lining residue qualifies. F5 is now a merge test -- a residue splits the cavity if truncating it merges components -- and PYR1 passes all five.
- **Shortcomings & failures:** Fifth metric of mine to be wrong in this project, and the first caught BEFORE being applied to anything.
- **What we learned:** Volume must be the LARGEST CONNECTED COMPONENT (Jannis); 145's free_volume summed every disconnected void. Measured inflation 1.00x on batches 1-3 so it changed nothing, but it is the correct definition.

### Latch vs lining, size-stratified

**Design - library** · Sep 15 · CPU local · `README 120`

- **How it computes:** min_conc from sd03's 641 clones regressed on WHERE a substitution sits -- latch (L117), gate (L87/A89) or pocket lining -- stratified by ligand heavy-atom count, with cLogP adjustment.
- **How we used it:** Jannis proposed testing the Whitehead/nitazene gate-latch-lock positioning idea on smaller ligands with more responsive first hits.
- **Inputs:** sd03, 641 clones with min_conc; Beltran-45 attempted as hold-out
- **Key result:** For SMALL ligands (<=16 HA) responsiveness tracks SHRINKING the latch: L117 dVolume -45.6 vs +14.6 A^3 (p = 0.0085), and L117 mutation is carried by 24% of responsive vs 5% of weak clones (p = 0.00022). The pocket LINING is null for small ligands (p = 0.64) but significant for large ones (p = 0.0099). The gate itself does nothing (p = 0.69). Responsive clones shrink AND polarise: L117->N x4, D x3, G x2, A, versus H/W/M in the weak set.
- **Shortcomings & failures:** POOLING HIDES IT COMPLETELY -- 22% vs 22%, p = 0.92 over all ligands. Every physical method this project built scores the LINING, which for small ligands carries no signal; that is a candidate single explanation for the whole run of small-ligand failures.
- **What we learned:** No out-of-library test exists: Beltran-45 has zero L117 substitutions and all 14 ligands are large, and the large-ligand lining analogue FAILS to replicate there (p = 0.175, direction reversed). min_conc conflates affinity with cell entry -- Spearman(min_conc, cLogP) = -0.374 -- but the confound runs AGAINST the finding (MH OR 6.37 adjusted vs 5.9 raw).

### Ligand-conditioning ceiling

**Design - benchmark** · Sep 15 · CPU local · `201`

- **How it computes:** Jaccard similarity of substitution sets within vs between ligands, with ligand-label permutation and a mut_lib control.
- **How we used it:** After five failed conditioning attempts, measure whether ANY ligand-conditioned method can work rather than trying a sixth.
- **Inputs:** sd03, 691 clones, 125 ligands with >=2 clones
- **Key result:** THE CEILING IS REAL: within-ligand Jaccard 0.0972 vs between-ligand 0.0201, a 4.8x enrichment, p < 0.0005 over 2000 permutations. Holds in every library separately (dsm 0.1132, tsm 0.0918, shuffle 0.4400).
- **Shortcomings & failures:** Identical re-isolates are only 0.6% of pairs and deduping leaves 4.57x, so it is not pseudo-replication. The six V164L+N167V ligands sit at 0.068 -- above random (0.023), below same-ligand (0.097): a generic solution with real per-ligand structure on top.
- **What we learned:** The five failures are failures of METHOD, not proof of impossibility. We have been conditioning on the wrong variable.

### Ligand size ceiling

**Design - benchmark** · Sep 14 · CPU local · `186`

- **How it computes:** Hit rate vs heavy-atom count over sd01's 194 hits and 3,172 documented failures, with cLogP, TPSA and rotatable bonds fitted jointly.
- **How we used it:** The premise the whole pocket-expansion project rests on, never previously checked against the screening record.
- **Inputs:** sd01, 3,366 parsed ligands
- **Key result:** The ceiling is real and sharp. The largest ligand that ever produced a sensor is 36 heavy atoms; of 222 tested at >= 40, NONE are hits (p = 1.9e-6 at the overall 5.8% rate). Hit rate falls 6-fold from the 15-25 atom plateau to the 30-40 band. Heavy atoms stays z = -5.22 with the confounds in the model.
- **Shortcomings & failures:** A ceiling is consistent with pocket-limitation AND with large compounds not reaching the yeast nucleus; this cannot separate them.
- **What we learned:** 945 documented failures sit in the 28-50 atom band -- targets where wild-type PYR1 demonstrably fails, so each carries its own negative control.

### Ligand-conditioned substitution priors

**Design - library** · Sep 1 · CPU local · `177, 178`

- **How it computes:** Similarity-weighted priors over substitutions: whole-molecule ECFP4/descriptor aggregation, and a per-pocket-position pharmacophore version.
- **How we used it:** Jannis's MSA analogy -- aggregate everything known about every screened ligand to BIAS the choice among sequences other filters allow, rather than to predict.
- **Inputs:** 194 sd03 ligands with clones; 181 with co-folded poses
- **Key result:** The whole-molecule prior beats the ligand-blind bar by +0.05 recall@20 (0.398 vs 0.345), five times 52's +0.010, and is STABLE across the weighting exponent rather than sitting on a knife edge. Paired: +0.053, t = 17.4, 49 of 50 splits positive.
- **Shortcomings & failures:** The control kills the MSA analogy: the entire gain comes from ligands WITH a close analog (+0.191 at Tanimoto >= 0.5) and it is NEGATIVE where none exists (-0.036 at < 0.2) -- the case a novel target is in. The per-position version fails at every shell radius and monotonically worse the more local it gets, because the pocket is 11.6 A long so a 7 A shell is the whole molecule, and a 3.5 A shell leaves too few ligands per position.
- **What we learned:** Gate it on max Tanimoto >= 0.25: +0.056 across 69 % of ligands, switched off for novel chemotypes. A data-density limit, not a flaw in the idea -- 18 position-specific priors cannot be estimated from 182 ligands.

### Backbone envelope vs cavity volume

**Pocket - structure** · Sep 1 · CPU local · `181, 182`

- **How it computes:** polyGly free volume inside a structurally-aligned wall hull, plus depth and depth/width of that envelope. Never cavity-derived.
- **How we used it:** Jannis: cavity volume conflates backbone with side chains, and if a donor's larger cavity is just smaller side chains that is cheaper to get by mutating PYR1.
- **Inputs:** PYR1 + 6 SRPBCC donors + 3 START lipid-transfer domains
- **Key result:** It overturned the donor ranking. 2PCS -- preferred throughout this project -- has a polyGly volume of 1173 against PYR1's 1184, DEAD EVEN: its 570 A^3 cavity against PYR1's 164 is entirely side chains. Same for 2NS9, 6AWV, 3OQU. The real difference is SHAPE: PYR1's envelope is nearly isotropic (d/w 1.25) while CERT/2E3M is 36.8 A deep at d/w 2.36.
- **Shortcomings & failures:** This metric was wrong twice before (143's circular envelope, 143's poly-Gly column reporting a vanished component as 0-3 A^3). 145 fixed both and this uses that version.
- **What we learned:** Since 8's limit on PYR1 is LENGTH not volume, the importable difference is a longer pocket, not a bigger one -- and no donor offers both. Design targets should be depth and d/w.

### Boltz-2 co-folding and affinity head

**Affinity - ML** · Aug 27-31 · GPU short_gpu · `139-142, 161-164; jobs 27978561, 27981866`

- **How it computes:** Structure + binding prediction from sequence and SMILES; affinity_probability_binary and affinity_pred_value.
- **How we used it:** Asked three separate questions: can it rank pocket VARIANTS, can it predict ligand TRACTABILITY, and can it supply poses for ligands with no crystal structure.
- **Inputs:** 718 sensor structures; 362 property-matched tractability models; 50 pose predictions
- **Key result:** Variant ranking: NO (within-ligand rho -0.028, and the binder classifier calls 90 % of real sensors non-binders). Poses: it finds the right pocket every time and builds the CLOSED receptor (gate RMSD 1.15-1.80 A), with a single consistent pose for about a quarter of ligands (97 of 362 under 1.0 A).
- **Shortcomings & failures:** Two silent failures cost whole arrays: a single trailing NUL byte in the shared a3m, and a ONE-RESIDUE mismatch between the a3m query and the input sequence, which makes it abandon the MSA and fold single-sequence at pLDDT 49 instead of 96. Neither errors.
- **What we learned:** Assert the fold (median pLDDT >= 80) and assert the a3m query equals the input sequence, before any downstream number is computed.

### Decoded true-negative label set (Park Fig 5)

**Design - benchmark** · Aug 31 · CPU local · `149, 156, 160, 166`

- **How it computes:** Programmatic decoding of a published figure: each substitution cell is a 2x2 block, one sub-cell per compound, positions anchored to the label glyphs.
- **How we used it:** Supplied the first true negatives in the project. Every metric before this was recall-only because the ground truth was positive-unlabeled.
- **Inputs:** 475 variants x 15 agrochemicals in the K59R background
- **Key result:** 7,045 TRUE NEGATIVES against 80 positives. 5,833 of them follow from the caption and the compound list alone; only 1,292 cells needed decoding. Quadrant-to-compound mapping verified with zero cross-contamination. Also corrected the benchmark itself: K59R was FORCED, not selected, and V81I is a benzothiadiazole hit rather than a mandipropamid one.
- **Shortcomings & failures:** Responsiveness is not affinity, and cross-reactivity is a separate axis this set does not resolve. 8 of the 25 residues are absent from the figure and are negative by omission.
- **What we learned:** Score a variant only against the ligand whose pose was modelled. Comparing a benoxacor responder against the mandipropamid pose asks about a molecule twice the size of the one it binds.

### Clash-relief scope test (four ligands, own poses)

**Design - scoring** · Aug 31 · CPU cutlerlab · `165, 166, 167`

- **How it computes:** Park's 475 scored against each compound's own pose, so that pose confidence and ligand chemistry vary independently.
- **How we used it:** Asked what the AUC 0.868 mandipropamid result actually measures.
- **Inputs:** mandipropamid (crystal) + 3 agrochemicals (predicted)
- **Key result:** Mandipropamid AUC 0.868 (p < 1e-4, 11 of 20 positives in the top 25); fludioxonil 0.550, benzothiadiazole 0.528, benoxacor 0.469, all at chance with ZERO positives in the top 25. The cause is not the pose: mandipropamid strains the unmutated pocket by +1501 REU while the three agrochemicals, at 13-18 heavy atoms against its 30, strain it by -6.5 to +36.4. They already fit, so there is no clash to relieve.
- **Shortcomings & failures:** The scope is narrower than 91 implied: it answers which mutation makes ROOM for a ligand too big for the pocket, and is silent on which mutation makes a pocket bind a ligand that already fits. Size does not predict strain -- WIN 55,212-2 is 32 heavy atoms and strains PYL2 by only +15.7.
- **What we learned:** Mandipropamid is the only large, clashing, positively-labelled ligand in the corpus, so the comparison cannot be made by adding a ligand. Isolate the pose variable instead (167).

### Forcing rule for library positions

**Design - library** · Aug 25-26 · CPU local · `116-119, 128, 153`

- **How it computes:** Two-part statistic: P(position mutated | ligand class) and P(residue | mutated, class), used to force a position into a library menu.
- **How we used it:** Asked whether a shallow menu at a productive position beats a deep menu at an unproductive one.
- **Inputs:** Tian sd03/sd07 coumarin and PFAS arms
- **Key result:** Forcing is a real lever at shallow positions, and V163W was callable in advance. On PFAS the headroom is POSITIONAL, not identity: 3,441x collapses to 192x once the <=6 substitution depth cap is applied to both sides.
- **Shortcomings & failures:** P(top | mutated) was partly circular -- 1.00 by construction wherever the library offered a single option.
- **What we learned:** The vocabulary is closed; the win is new positions. Confirmed independently on Beltran-45, where Tian's frequency prior saturates exactly at the 67 % vocabulary ceiling.
...
**Scaffold - design** · Sep 1-14 · GPU short_gpu · `183, 187, 188, 189, 193; jobs 28380382/3`

- **How it computes:** Diffusion scaffolding around the four HAB1-interface segments held as a fixed motif, with HAB1 present as a steric constraint and sampled scaffold segment lengths.
- **How we used it:** The only untested route to a genuinely larger BACKBONE, after 106 showed no natural donor offers one.
- **Inputs:** PYR1 chain A + HAB1 chain B from 3QN1, auth-numbered
- **Key result:** The motif scaffolds easily -- 0.1 A CA deviation over the fixed positions, all four segments present in 10/10. Batch 3 (no RASA conditioning) gives 7/10 designs with an enclosed cavity and ALL SEVEN exceed PYR1's 1184 A^3 backbone envelope, best 1353 (+14%).
- **Shortcomings & failures:** RASA conditioning SUPPRESSES the pocket, which Jannis diagnosed from the design and the code confirms: rasa.py bins with a 1.4 A water probe, so a pocket-lining residue is genuinely high-RASA and select_buried (bin 0) is satisfied by packing protein around it. buried 1/10, none 7/10, exposed 0/10. Designs are still loose (Rg 22-27 A vs PYR1's 15.0) and the sequence is a low-complexity placeholder (71% A/S/T/G/P/V), so every volume is a polyGly number.
- **What we learned:** Four invocation traps, all recorded in the script header: comma-separated contigs, /0 chain breaks, top-level specification fields, and quoted comma values. Feed an auth-numbered PDB -- RFd3's CIF parser reads label numbering and 3QN1 chain A carries a -2 offset.

### Pose-check MD

**Structure - validation** · Sep 1 · GPU short_gpu · `179, 180; job 27992285`

- **How it computes:** 6 ligand/receptor systems x 3 replicates x 20 ns; ligand heavy-atom RMSD to the starting pose with the protein core superposed.
- **How we used it:** Asks whether MD retains a predicted pose, since most poses this project trains on were predicted once and 93a showed co-folding places a ligand in the pocket regardless of whether it belongs.
- **Inputs:** ABA, mandipropamid (crystal AND predicted), WIN, fludioxonil, benoxacor
- **Key result:** IN FLIGHT, 8 of 18 complete. All six systems built at 39,507-43,893 atoms with no parmchk2 ATTN flags -- no guessed parameters, which matters because a guessed torsion is indistinguishable from a bad pose in an RMSD trace.
- **Shortcomings & failures:** Six build attempts were needed, every failure a silent format mismatch (explicit hydrogens, sanitize=False, MDL aromatic bond type 4). Recorded in the script so they are not rediscovered.
- **What we learned:** Two paired comparisons carry it: mandipropamid crystal vs predicted, and fludioxonil (0.71 A seed spread) vs benoxacor (3.27 A).

### RFdiffusion3 motif scaffolding

**Scaffold - design** · Sep 1 · GPU short_gpu · `183; job 27994380`

- **How it computes:** Diffusion scaffolding around the four HAB1-interface segments held as a fixed motif, with wide linker ranges so scaffold length and therefore envelope can vary.
- **How we used it:** Revisits 7's rejection. That objection was about the two-state switch, NOT about cavity generation, and 85 removed its discriminating power -- no design of any kind can now be shown to cycle.
- **Inputs:** PYR1 chain A, residues 1-181; motif A60-63/A84-89/A115-117/A148-166
- **Key result:** QUEUED behind the pose-check MD. Residue numbering asserted before submission: all 19 interface residues present with expected identities, gaps only at 69-70 which fall in a rebuilt linker.
- **Shortcomings & failures:** Fixing the C-lobe span freezes F159/V163/V164, which are pocket wall, so depth gain is one-sided. Inherent to preserving the interface. A prior RF3 attempt in the sibling project failed on a config API mismatch.
- **What we learned:** Scored on ENVELOPE not cavity volume (106). Batch 2 is specified with Jannis's minimal loop anchors: 54 residues covering all 19 interface positions, against 139 for the literal extend-to-structure reading.
...
### Latch advance geometry

**Design - structural** · Sep 15 · CPU cutlerlab · `204; job 28400142`

- **How it computes:** Rigid-body translation of the latch (115-117) toward a placed ligand until clash, with and without the Leu117 side chain, to test whether Leu117 sets the floor on latch closure.
- **How we used it:** To turn 120's correlation into a physical predictor.
- **Inputs:** 3QN1 chain A, 182 sd03 ligands
- **Key result:** VOID. Every advance is 0.00 A for every ligand in both conditions.
- **Shortcomings & failures:** Three defects, all mine: (1) the latch is covalently bonded to residues 114/118 at 1.33 and 1.31 A, and the clash test is a vdW criterion (~2.9 A C-C), so the bonded neighbours are a permanent clash and translation aborts on step 1; (2) only 70/182 ligands placed at all -- a principal-axis alignment x 12-orientation grid is far too coarse; (3) THE CONTROL PASSED SPURIOUSLY -- it required |gain| < 0.6 for ABA, which the all-zero failure mode satisfies trivially.
- **What we learned:** A control must be able to FAIL in the direction the bug produces. And the model is wrong independent of the code: rigidly translating a loop embedded in a continuous chain is a motion the protein cannot make. Static reformulation (free volume between ligand and latch backbone, +/- the Leu117 side chain) not yet run.

### Volume-deficit fill matching

**Design - library** · Sep 15 · CPU local · `200`

- **How it computes:** Rank substitutions by whether their side-chain volume change matches the deficit a ligand leaves against ABA's 249 A^3 footprint, evaluated leave-one-LIGAND-out on sd03.
- **How we used it:** Eugenol under-fills ABA by 87 A^3 and its hit grows by +52.6 A^3; the fifth attempt at ligand conditioning.
- **Inputs:** sd03, 181 ligands with computed vdW volumes
- **Key result:** CLOSED. The mechanism is real -- Spearman(deficit, mean dVolume) = +0.296, p = 5.1e-5 -- but no re-ranking converts it into recall. Hard sign-partition: recall@20 0.365 -> 0.300, 38 wins/80 losses, p = 3e-4. Parameter-free tie-break: 0 wins/6 losses. No stratum of |deficit| rescues it.
- **Shortcomings & failures:** Eugenol itself goes blind@20 0.00 -> 1.00, which is the anecdote that motivated the idea and is not a method.
- **What we learned:** Distinguished in the script header from dead cavity-match (69, scored INSIDE a menu) and the retracted size prior (115d, which SUBSET the training clones where this only re-orders them). Permutation control passes at p = 0.010, so the deficit carries information we cannot spend.

### Crystallographic pocket-water position prior

**Design - library** · Sep 15 · CPU local · `202`

- **How it computes:** Rank the 18 mutable positions by contact with ordered pocket waters in 3QN1/3K3K, scored against blind position frequency on Beltran-45.
- **How we used it:** fa_sol gave the largest non-volume AUC gap in the project (+0.133 on fludioxonil) and 114e's anti-complementarity points at desolvation.
- **Inputs:** 3QN1, 3K3K, 4WVO, 8EY0
- **Key result:** NULL. Worse than blind frequency at every k (-0.10 to -0.30) and indistinguishable from permuted water counts (p = 0.646).
- **Shortcomings & failures:** Driest positions carry more clones (117 vs 73), the anti-complementarity direction, but r = -0.170, p = 0.499 at n = 18 -- no power.
- **What we learned:** Chain and numbering resolved BY IDENTITY ASSERTION, not assumed: 3QN1 and 3K3K both 18/18 on chain A auth; 4WVO 14/18 and 8EY0 12/18 are their engineered mutations.

### Ligand-conditioned priors (four attempts)

**Design - library** · Aug 24 - Sep 14 · CPU local · `177, 178, 185, 194`

- **How it computes:** Priors over substitutions conditioned on the query ligand: similarity lookup, whole-molecule aggregation, per-pocket-position pharmacophore, and ligand SIZE.
- **How we used it:** Asked whether knowing the ligand beats the ligand-blind frequency prior on a held-out set.
- **Inputs:** 194 sd03 ligands; Beltran-45 and sd07 as test sets
- **Key result:** All four FAIL out of sample. The ligand-blind frequency prior remains unbeaten (recall@20 0.429, @40 0.660 on Beltran-45).
- **Shortcomings & failures:** The size arm looked like a win -- +0.142 recall@20, sign-flip p 0.0017, 10 wins 0 losses -- and was RETRACTED by two controls: the small arm loses 0/7 on sd07, and 400 random subsets of equal size reproduce the large-arm gain (P = 0.21). The paired test was correct but measured SUBSETTING, and a one-arm test cannot see a routing effect.
- **What we learned:** Control against a RANDOM subset of the same size before crediting any conditioning. And no clean small-ligand test exists: every Tian downstream library was drawn from the sd03 vocabulary (sd07 100%, sd08 96%, sd09 67%), so check the LIBRARY's provenance, not the compounds'.

### Thermodynamic integration (TI)

**Affinity - energetics** · Aug 22-24 · GPU · `85-92; jobs 27725295 and predecessors`

- **How it computes:** Alchemical free-energy difference along a lambda path with softcore; two legs give a selectivity ddG.
- **How we used it:** Asked whether TI can rank PYR1 point mutations well enough to choose between them.
- **Inputs:** PYR1 + ABA / mandipropamid; 85-88, 90-92
- **Key result:** A working Amber TI pipeline with six setup traps documented. The pilot ran 72/72 and the quadruple 48/48. The crystal arm gives the right SIGN and NO CALL at +-2.34 kcal/mol against an effect of ~1.
- **Shortcomings & failures:** The docked arm is structurally invalid -- the ligand collapses into deleted F108's volume, which breaks the whole dock-into-an-enlarged-pocket workflow. The pilot's own error bar was 20x too small until a regex double-counting frames was found.
- **What we learned:** 8.3 GPU-days per variant pair is the real cost, which is what moved the project off affinity prediction and onto library enrichment (47).

### Umbrella sampling of the open/closed coordinate

**Conformation - energetics** · Aug 25-28 · GPU short_gpu · `111-113, 127, 136, 138; jobs 27854551`

- **How it computes:** 62 windows along P88 CA - R116 CA with WHAM, forward and reverse seeded, holo and apo.
- **How we used it:** The last route to the open/closed dG that unbiased MD cannot reach (62), and so to the switching cost of a designed pocket.
- **Inputs:** S1/S2/S9/S10 factorial seeds; 1.24 us biased
- **Key result:** FAILS its own pre-registered test. Forward and reverse disagree by 3.22 kcal/mol holo and 2.74 apo against a half-split drift bar of 0.22 and 1.05, and the SIGN of the tilt flips between seedings (+4.61 vs -3.29). Calibration also fails on its own terms (ddG -1.90 where a pass needs positive).
- **Shortcomings & failures:** My own 70/127 repair destroyed the two-directional seeding, which 81b suspected and 85 confirmed. Separately, the seeds came from only 5 of the 11 finished factorial trajectories (see the extraction bug below).
- **What we learned:** A 15x miss with a sign flip is protocol, not sampling length. Do not rerun. Switch cost is a Y2H question.
