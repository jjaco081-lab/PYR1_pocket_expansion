...
## Appendix: graft-direction screen (2026-08-20, lower priority)

Run opportunistically while SLURM jobs occupied the queue. Recorded here rather
than in the main table because it is an exploratory branch, not an established
method — and because the literature baseline for pocket grafting is poor.

**The inversion.** Rather than enlarging PYR1's pocket, take a same-fold relative
that already has a large pocket and move PYR1's machinery onto it. The machinery is
19 residues in four discontinuous segments (the HAB1 interface, README §9c), 15 of
which are also the homodimer interface.

Screen: `scripts/79_graft_geometry.py` — superpose on the shared core *excluding*
the 19, then measure per-segment coverage, indels and local RMSD.

| PDB | what it is | cavity Å³ | iface | gate RMSD | note |
|---|---|---|---|---|---|
| **2BK0** | Api g 1, celery allergen (Bet v 1 / PR-10) | 344 | 19/19 | **2.2 Å** | best gate of the large-cavity set |
| 2PCS | CoxG | **570** | 19/19 | 5.1 Å | biggest pocket; 1 gate insertion |
| 2NS9 | — | 455 | 19/19 | 6.9 Å | worst gate |
| 6AWV | — | 319 | 17/19 | 6.8 Å | 2 gate residues missing |
| 4DSB / 3OQU | PYL homologs | 162 / 204 | 19/19 | 1.2 / 1.0 Å | positive control — 0 indels, ~1 Å |

⚠ **The honest caveat.** Core RMSD is 3.4–4.0 Å for every large-cavity candidate at
10–17 % sequence identity. The segment RMSDs are measured against a superposition
that is itself 3.5 Å off, so "graftable" here means *rebuildable with a template*,
not transplantable. The PYL controls at 1.0–1.5 Å core RMSD show what a real
transplant looks like, and none of the large-cavity candidates are close to it.

⚠ **A second risk the screen cannot see.** Geometry says nothing about whether a
grafted construct would still *cycle*. README §24/§29 show the gate–latch staple is
intermittent in the closed monomer and is clamped by the partner — the mechanism is
cooperative, not modular. Every piece could be placed correctly and the result never
close.

**Bug caught during the run:** the first insertion metric compared the target span
against the *number of selected positions* rather than the query span. The interface
residues are not contiguous in PYR1 either (the C-lobe picks 8 positions out of
148–166), so it reported an 11-residue "insertion" for PYL homologs at alnTM 0.97.

**Fetch completed**: all 66 solved domains at alnTM ≥ 0.5 are now on disk (the 5
failures were AlphaFold models). Two previously unfetched domains carry ligands —
**3H3Q** (CERT START domain with ceramide, 27 heavy atoms) and **4QDC** (a steroid,
21 heavy). Their cavities are not yet measured; that needs `08 --stage compute`.
